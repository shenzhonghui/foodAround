...
;(function($) {
    ...
    //重置zepto的show方法，防止有些人引用的版本中 show方法操作opacity属性影响动画执行
    $.fn.show = function() {
        var elementDisplay = {};
...
+ function($) {
    ...
    var showTab = function(tab, tabLink, force) {
        ...
        // Find tab's view
        var viewContainer;
        if (newTab.hasClass(defaults.viewClass)) viewContainer = newTab[0];
        else viewContainer = newTab.parents('.' + defaults.viewClass)[0];
...
+ function($) {
    ...
    $.modal = function(params) {
        ...
        if (defaults.modalTemplate) {
            ...
            var animatedClass = params.animatedClass ? params.animatedClass : '';
            var textHTML = params.text ? '<div class="modal-text">' + params.text + '</div>' : '';
            var afterTextHTML = params.afterText ? params.afterText : '';
            var noButtons = !params.buttons || params.buttons.length === 0 ? 'modal-no-buttons' : '';
            var verticalButtons = params.verticalButtons ? 'modal-buttons-vertical' : '';
            modalHTML = '<div class="modal ' + noButtons + ' '+animatedClass+'"><div class="modal-inner">' + (titleHTML + textHTML + afterTextHTML) + '</div><div class="modal-buttons ' + verticalButtons + '">' + buttonsHTML + '</div></div>';
        }
    ...
    $.openModal = function(modal) {
        ...
        var isPopupLeft = modal.hasClass('popup-left');
        ...
        if (isPickerModal) {
            ...
            // $(defaults.modalContainer).addClass('modal-overlay-visible');
        ...
        if (!isLoginScreen && !isToast) {
            ...
            if($('.modal-overlay').length>0){
                overlay.addClass('modal-overlay-visible-P0');
            }
        }
    ...
    function handleClicks(e) {
        ...
        if (clicked.hasClass('popup-overlay')) {
            ...
            if ($('.popup.modal-in').length > 0) $.closeModal('.popup.modal-in');
            
        }
        //关闭floater
        if(clicked.hasClass('popup-floater')){
            if ($('.popup.modal-in').length > 0 && defaults.closeOverlayFloat) {
                $.closeModal('.popup.modal-in');
                $(this).remove();
            }
        }
    ...
    var defaults = $.modal.prototype.defaults = {
        ...
        closeOverlayFloat:true //close all  floater in the overlay
        // closeOverlayClass:'.slowClick-cancel' //close all  floater in the overlay
    };
    $(function() {
        $(document).on('click', ' .modal-overlay, .popup-overlay, .close-popup, .open-popup, .open-popover, .close-popover, .close-picker, .popup-floater', handleClicks);
...
+ function($) {
    ...
    $.fn.datePicker = function(params) {
        return this.each(function() {
            ...
            var inputValue = $(this).val();
            if(params.value === undefined && inputValue !== "") {
                // params.value =inputValue.split(" ")[0].split("-");
                params.value =inputValue.split("-");
            }
            var p = $.extend(defaultsdate, params);
            $(this).picker(p);
        });
    };
}($);
    ...
    $.fn.timePicker = function(params) {
        return this.each(function() {
            ...
            var defaultstime = {
                ...
                onChange: function(picker, values, displayValues) {
                    // console.log(picker);
                    // picker.cols[0].setValue(picker.cols[0].value);
                    // picker.cols[2].setValue(picker.cols[1].value);
                },
                formatValue: function(p, values, displayValues) {
            ...
            var p = $.extend(defaultstime, params);
            $(this).picker(p);
        });
    };
}($);
...
+ function($){
    ...
    $.resizableTextarea = function (textarea) {
        textarea = $(textarea);
    ...
    $.initPageResizableTextareas = function (pageContainer) {
        pageContainer = $(pageContainer);
        var textareas = pageContainer.find('textarea.resizable');
        textareas.each(function () {
            $.resizableTextarea(this);
        });
    };
}($);
...
+ function($) {
    'use strict';
    var defaults;
    var ShowByWord=function(container,params){
        var w=this;
        params=params||{};
        for (var def in defaults) {
            if (typeof params[def] === 'undefined') {
        ...
        w.params = params;
        var d = $(container),s = [],i = w.params.content,f=i.length,l=w.params.fadeSpeed;
        d.empty();
        d.attr("data-time", f * w.params.speed);
        for (var p = 0; p < f; p++) s.push(i[p]);
        $.each(s,function(inx,item){
            var h=inx* w.params.speed;
            d.append('<span style="opacity:0;">' +item+ '</span>');
            // setTimeout(function() {
                if(w.params.type=='show'){
                    d.children("span:nth-child("+(inx+1)+")").show();
                }else if(w.params.type=='fade'){
                    d.children("span:nth-child("+(inx+1)+")").addClass('fadeIn').css({
                        "animation-delay":  h+ "ms",
                        "-o-animation-delay": h+ "ms",
                        "-webkit-animation-delay": h+ "ms",
                        "-moz-animation-delay": h + "ms",
                        "-ms-animation-delay": h+ "ms",
                        "animation-duration": l + "ms",
                        "-o-animation-duration": l + "ms",
                        "-webkit-animation-duration": l + "ms",
                        "-moz-animation-duration": l + "ms",
                        "-ms-animation-duration": l + "ms"
                    })
                }
            // }, inx* w.params.speed)
        })
    ...
    $.fn.showByWord=function(params){
        return this.each(function() {
            var $this = $(this);
            if(!$this[0]) return;
            new ShowByWord(this,params);
        })
    };
    defaults=$.fn.showByWord.prototype.defaults={
        content: "",
        speed: 100,
        type: "fade",
        fadeSpeed: 500,
    ...
    //         $.each(s, function(inx,item) {
    //             d.append('<span style="display: none;">' + item + "</span>"), setTimeout(function() {
    //                 "show" == t.type ? d.children("span:nth-child("+inx+")").show() : "fade" == t.type && d.children("span:nth-child("+inx+")").show()
    //             }, inx * t.speed)
    //         }), setTimeout(function() {
    //             t.finished()
    //         }, i.length * t.speed)
...
/*常用方法*/
+ function($) {
    Date.prototype.formatDateTime=function(dateFormat){
    ...
    */
    $.jsonSort=function(arr,field,reverse){
...
            return obj;
    ...
    /*ajaxpost请求*/
    $.XHR=function(url,para,successOK,cancelOK){
        $.ajax({
            type: "POST",
            url: url,
            timeout: 5e3,
